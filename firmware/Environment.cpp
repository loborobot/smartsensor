...
void Environment::execute(int flo, int vol){ // init program
  String data = strucData(flo, vol);
  //if(conn.statusConn==true && conn.statusServer==true){
    //while(flat == -1){updateSensor();}
    //if(flat == 0){
  conn.printData(data);
  if(conn.sendQueryData(data)){
    sendData = true;
  }
  else{
    //conn.connect();
...
uint8_t Environment::readDataDHT() // temperature and humidity function
{
	// BUFFER TO RECEIVE
	uint8_t bits[5];
	uint8_t cnt = 7;
...
int Environment::readDataLDR() // LDR function
{
...
char* Environment::readDataUV(){ // UV function
...
char* Environment::readDataSound(){
...
char* Environment::readDataFlowmeter(int flat){
...
char* Environment::readDataCO2(){ //cos2 function
  _CO2 = analogRead(CO2_PIN);
  return convertF2C(_CO2);
}

char* Environment::readDataNO2(){ //no2 funcion
  _NO2 = analogRead(NO2_PIN);
  return convertF2C(_NO2);
}

// strtc data function
String Environment::strucData(int flo, int vol){
  int i = 0;
  String data;
...
char* Environment::convertF2C(float val){
