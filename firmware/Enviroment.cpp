...
void Enviroment::begin(){ //init variables
...
void Enviroment::execute(int flo, int vol){ // init program
...
uint8_t Enviroment::readDataDHT() // temperature and humidity function
{
	// BUFFER TO RECEIVE
	uint8_t bits[5];
	uint8_t cnt = 7;
...
int Enviroment::readDataLDR() // LDR function
{
...
char* Enviroment::readDataUV(){ // UV function
...
char* Enviroment::readDataFlowmeter(int flat){
...
char* Enviroment::readDataCO2(){ //cos2 function
  _CO2 = analogRead(CO2_PIN);
  return convertF2C(_CO2);
}

char* Enviroment::readDataNO2(){ //no2 funcion
  _NO2 = analogRead(NO2_PIN);
  return convertF2C(_NO2);
}

// strtc data function
String Enviroment::strucData(int flo, int vol){
  int i = 0;
  String data;
...
char* Enviroment::convertF2C(float val){
